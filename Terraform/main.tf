...
resource "launchdarkly_project" "leaddev" {
  key  = "ld-demo-leaddev"
  name = "ld-demo-leaddev"
...
resource "launchdarkly_feature_flag" "qrcode" {
  project_key = launchdarkly_project.leaddev.key
  key         = "qrcode"
  name        = "QR Code"
...
output "LaunchDarkly_API_Key" {
  value = launchdarkly_project.leaddev.environments[0].api_key
  sensitive = true
}

output "LaunchDarkly_Client_Side_Key" {
  value = launchdarkly_project.leaddev.environments[0].client_side_id
  sensitive = true
}
